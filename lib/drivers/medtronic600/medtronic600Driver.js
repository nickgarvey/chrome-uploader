...
class DeviceCharacteristicsResponse extends TransmitPacketResponse {
  ...
  static buildForCnl(pumpSession) {
    ...
    payload.write(pumpSession.linkMAC, 0xA, 8, 'hex');
    payload.write('010101000000660001000A00020000FFFF0701000000', 0x12, 22, 'hex');
    ...
    const deviceCharacteristics =
      new DeviceCharacteristicsResponse(responsePacket.payload, pumpSession);
    const ngpRequest =
      new NGPRequest(
        NGPMessage.COMMAND_TYPE.COMMAND_RESPONSE,
        pumpSession, deviceCharacteristics.payload,
      );
    debug(`*** NGP REQUEST: ${ngpRequest.payload.toString('hex')}`);
    return ngpRequest;
  }
...
class DeviceCharacteristicsCommand extends TransmitPacketRequest {
  ...
  constructor(pumpSession, responseType = DeviceCharacteristicsResponse) {
...
class MM600SeriesDriver extends BCNLDriver {
  ...
  setPumpMACFromSerial(serialNumber) {
    // TODO: Get the real thing from NGP:convertSerialToMacID()
    debug(`Setting pump serial to ${serialNumber}`);
    this.pumpSession.pumpMAC = '0023F745EE102495';
    // TODO: build the read temp key from linkMAC and pumpMAC
    this.pumpSession.key = Buffer.from('7a1c108206f72300952410ee45f72300', 'hex');
  }
